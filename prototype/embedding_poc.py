"""
Embedding Proof of Concept for LTM

Tests semantic retrieval and auto-linking WITHOUT touching the real database.
Uses sentence-transformers with a lightweight CPU-friendly model.

Usage:
    uv run --with sentence-transformers python prototype/embedding_poc.py
"""
...
embedder = None
MODEL_NAME = "all-MiniLM-L6-v2"  # 80MB, fast on CPU, good quality
...
def demo():
    """Run a demonstration of the embedding system."""
    print("=" * 60)
    print("EMBEDDING PROOF OF CONCEPT")
    print("=" * 60)

    # Use a test database in prototype folder
    db_path = Path(__file__).parent / "test_memories.db"
    if db_path.exists():
    ...
    for mem in test_memories:
        ...
        print(f"  [{mem.id}] {mem.content[:50]}... ({embed_time:.3f}s)")

    print(f"\nTotal embedding time: {total_embed_time:.2f}s")
    print(f"Average per memory: {total_embed_time/len(test_memories):.3f}s")
