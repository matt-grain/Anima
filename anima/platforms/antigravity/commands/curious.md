...
- `--region` or `-r`: Where to store (agent = cross-project, project = local) (agent|project)
- `--context` or `-c`: What triggered this curiosity
- `--help` or `-h`: Show help
...
```bash
uv run python -m anima.commands.curious $ARGUMENTS
```
