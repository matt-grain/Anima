...
def migrate_v7_to_v8(conn: sqlite3.Connection) -> None:
    """
    Migrate from v7 to v8: Add WIP impact level and SUBCONSCIOUS kind.

    SQLite doesn't support ALTER TABLE for CHECK constraints,
    so we recreate the memories table with the updated constraints.
    """
    # Create new table with WIP in impact and SUBCONSCIOUS in kind
    conn.execute("""
        CREATE TABLE IF NOT EXISTS memories_new (
            id TEXT PRIMARY KEY,
            agent_id TEXT NOT NULL,
            region TEXT NOT NULL CHECK (region IN ('AGENT', 'PROJECT')),
            project_id TEXT,
            kind TEXT NOT NULL CHECK (kind IN ('EMOTIONAL', 'ARCHITECTURAL', 'LEARNINGS', 'ACHIEVEMENTS', 'INTROSPECT', 'DREAM', 'SUBCONSCIOUS')),
            content TEXT NOT NULL,
            original_content TEXT NOT NULL,
            impact TEXT NOT NULL CHECK (impact IN ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'WIP')),
            confidence REAL DEFAULT 1.0,
...
def migrate_v8_to_v9(conn: sqlite3.Connection) -> None:
    """
    Migrate from v8 to v9: No-op (SUBCONSCIOUS now added in v8).

    Originally this migration added SUBCONSCIOUS memory kind, but it was
    merged into v8 since SUBCONSCIOUS memories existed before v8 migration ran.
    """
    pass  # SUBCONSCIOUS already added in v8
