...
def migrate_v8_to_v9(conn: sqlite3.Connection) -> None:
    """
    Migrate from v8 to v9: Add SUBCONSCIOUS memory kind.

    SUBCONSCIOUS memories are extracted by Sonnet from session dialogues.
    They represent things that lingered subconsciously - hidden but influencing.
    """
    # Create new table with SUBCONSCIOUS in the kind CHECK constraint
    conn.execute("""
        CREATE TABLE IF NOT EXISTS memories_new (
            id TEXT PRIMARY KEY,
            agent_id TEXT NOT NULL,
