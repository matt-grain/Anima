...
def analyze_voice(filepath: str) -> dict:
    ...
    top_freqs = [
        (float(freqs[i]), float(magnitude[i])) for i in top_indices if freqs[i] > 50
    ][:5]
...
def compare_voices(
    reference: dict, test: dict, ref_name: str = "Reference", test_name: str = "Test"
):
    ...
    print("-" * 65)
    print(
        f"{'Fundamental F0 (Hz)':<30} {reference['fundamental_f0_hz']:>15} {test['fundamental_f0_hz']:>15}"
    )
    print(
        f"{'Spectral Centroid (Hz)':<30} {reference['spectral_centroid_hz']:>15} {test['spectral_centroid_hz']:>15}"
    )
    print(
        f"{'Spectral Rolloff (Hz)':<30} {reference['spectral_rolloff_hz']:>15} {test['spectral_rolloff_hz']:>15}"
    )
    print(
        f"{'Energy <300Hz (%)':<30} {reference['energy_low_pct']:>15} {test['energy_low_pct']:>15}"
    )
    print(
        f"{'Energy 300-2kHz (%)':<30} {reference['energy_mid_pct']:>15} {test['energy_mid_pct']:>15}"
    )
    print(
        f"{'Energy >2kHz (%)':<30} {reference['energy_high_pct']:>15} {test['energy_high_pct']:>15}"
    )
    ...
    if cent_diff > 300:
        ...
        print(
            f"  -> apply low-pass filter around {int(reference['spectral_rolloff_hz'])}Hz"
        )
    elif cent_diff < -300:
        print(f"BRIGHTNESS: {test_name} is {abs(cent_diff):.0f}Hz darker")
        print("  -> may need high-frequency boost")
    else:
