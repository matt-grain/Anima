...
| 🟡 | **Global `/clear` achievement detection** (claude `setup_hooks`) | The global install now wires the SessionStart **`clear`** matcher so **memories re-inject after `/clear`** (fixed). What's still local-only is **`detect_achievements`** on `/clear` — global uses HTTP shims and there's no detect-achievements route, so the global `clear` matcher only re-injects, it doesn't auto-detect achievements. Fix: add a detect-achievements HTTP route + shim. Matcher-set parity pinned by `TestSetupConfigMatrix::test_global_and_local_session_start_matchers_are_consistent`. |
